...
// handleAppConfig returns read-only, app-wide settings the UI needs. The
// sites/streams flags tell it which sections exist: it hides the ones that
// are switched off and lands on the first section that is actually there.
func (s *Server) handleAppConfig(w http.ResponseWriter, r *http.Request) {
	writeJSON(w, http.StatusOK, map[string]any{
		"defaultReloadOnSave": s.cfg.DefaultReloadOnSave,
		"sites":               s.cfg.SitesEnabled,
		"streams":             s.cfg.StreamsEnabled,
	})
}
