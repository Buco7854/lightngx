...
import { Btn, Input, Modal, ModalActions } from "./ui";
  ...
  return (
    <ConfirmContext.Provider value={ask}>
      <PromptContext.Provider value={prompt}>
        ...
        {confirmP && (
          <Modal
            title={confirmP.opts.title}
            hint={confirmP.opts.message}
            size="sm"
            icon={
              <span
                className={`flex h-9 w-9 items-center justify-center rounded-xl ${
                  confirmP.opts.danger ? "bg-danger-soft text-danger-ink" : "bg-accent-soft text-accent-ink"
                }`}
              >
                {confirmP.opts.danger ? <AlertTriangleIcon size={18} /> : <InfoIcon size={18} />}
              </span>
            }
            onClose={() => closeConfirm(false)}
          >
            <div className="flex flex-wrap justify-end gap-2.5">
              <Btn onClick={() => closeConfirm(false)} autoFocus>
                {t.cancel}
              </Btn>
              <Btn
                variant={confirmP.opts.danger ? "danger-solid" : "primary"}
        ...
        {promptP && (
          <Modal title={promptP.opts.title} size="sm" onClose={() => closePrompt(null)}>
            {(() => {
              ...
              return (
                ...
                >
                  {o.label && (
                    <label className="mb-1.5 block text-[13px] font-medium text-dim">{o.label}</label>
                  )}
                  <Input
                    ref={inputRef}
                    type={o.type}
                  ...
                  {o.confirm && (
                    <>
                      <label className="mt-3.5 mb-1.5 block text-[13px] font-medium text-dim">
                  ...
                  {mismatch && (
                    <p className="mt-2 mb-0 text-[13px] text-danger-ink">{t.passwordMismatch}</p>
                  )}
                  <ModalActions>
                    <Btn type="button" onClick={() => closePrompt(null)}>
                      {t.cancel}
                    </Btn>
